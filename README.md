...
| **Pipe Ativo** | Tabela com ordenação, filtros persistentes na URL, busca, mostrar/ocultar colunas, redimensionar, paginação server-side, seleção múltipla e bulk actions, header fixo, exportação CSV/Excel, views salvas. **Quick edit** de status, responsáveis, próxima ação e follow-up direto na linha. |
