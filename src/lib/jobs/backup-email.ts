...
export async function runBackupEmail(opts: { trigger: "scheduled" | "manual"; bucket?: string; userId?: string | null; recipients?: string[] }): Promise<{ ran: boolean; runId?: string; error?: string }> {
  ...
  try {
    ...
    const sent = await sendMail({ to: recipients, subject: `[Leto SS] Backup do pipe · ${stamp}`, text, attachments: [{ filename, content: buffer, contentType: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" }] });
    await prisma.jobRun.update({ where: { id: run.id }, data: { status: "OK", finishedAt: new Date(), bytes: buffer.length, detail: `${sent.provider}${sent.id ? ` ${sent.id}` : ""} · ${summary}` } });
